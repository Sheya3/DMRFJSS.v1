...
public class MultipleTreeRuleTest{
	 ...
	 public static final long simSeed = 968356;
	    ...
	    public MultipleTreeRuleTest(String trainPath, RuleType ruleType, int numRuns,
		...
		public void writeToCSV() {
			...
			// E:\download\grid\MTGP-tugboat-dynamic\energyConsumption-0.75-1.5test\missing-0.85-4.0.csv

	        List<TestResult> testResults = new ArrayList<>();

	        //for test: which machines are choosen by routing rule, CCGP. Scenario: run1, rule in generaiton 51 numRuns
//	        for (int i = 0; i < numR/uns; i++) {
...
//	            result.setGenerationalTimeStat(MultipleTreeResultFileReader.readTimeFromFile(timeFile));
	      ...
	      /*      File badrunsFile = new File(trainPath + "job." + i + ".BadRun.csv");
				...
				//"E:\download\grid\MTGP-main\MTGP-tugboat-dynamic1\energyConsumption-0.95-1.5-60\job.0.out.stat"
				File sourceFile = new File(trainPath ,"job." + i + ".out.stat");
//				File sourceFile = new File(trainPath + "job." + i + ".out.stat");  //this file keeps the rule
//				TestResult result = TestResult.readFromFile(sourceFile, ruleType, numTrees);
				TestResult result = MultipleTreeTestResult.readFromFile(sourceFile, ruleType, numTrees);
...
//						generationalRules[0].calcFitness(  //calcFitnessin calcFitness(), it will check which one is routing/sequencing rule
//								result, null,
//								testSet, generationalRules[1], objectives,j);
...
//						"TrainTime," +
//						" AveSeqRulesize, AveRouRuleSize, AveRuleSize"
				);
...
//	                                numUniqueTerminals + ",");
		...
		public static void main(String[] args) {
			int idx = 0;
			String trainPath = args[idx]; // /Users/feigeliu/Documents/Code/MTGP-main/out.stat
	        idx ++;

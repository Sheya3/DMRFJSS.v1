...
public class ATC extends AbstractRule {
    ...
    public void calcSlackNorm(OperationOption op, List<WorkCenter> workCenterset, SystemState systemState) {
        slackNorm = 0.0d;
        for (int i = 0; i < workCenterset.size(); i++) {
            ...
            List<OperationOption> queue = workCenter.getQueue();
            for (OperationOption o : queue) {
                slackNorm += slackNorm+o.getProcTime()/ queue.size()* k;
            }
        }
        slackNorm = slackNorm/workCenterset.size();
    }
    ...
    public double priority(OperationOption op, List<WorkCenter> workCenterset, SystemState systemState) {
        calcSlackNorm(op, workCenterset, systemState);
        calcExpWaitingTime(op);

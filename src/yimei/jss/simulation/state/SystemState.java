...
//    double DSTB;
    double[][] DSTS;
    double[][] DSTE;
    public double[][][] SailingPT;
    double[][] Machine_Horse_C0_C1_VTi;
...
//        this.DSTB=DSTB;
        this.DSTS=DSTS;
        this.DSTE=DSTE;
        this.SailingPT=SailingPT;
        this.Machine_Horse_C0_C1_VTi=Machine_Horse_C0_C1_VTi;
        this.idleEC = idleEC;
        this.processingEC = processingEC;
    }
...
//                       double DSTB,
        ...
        this(clockTime, new ArrayList<>(), new LinkedList<>(), new ArrayList<>(),
//                DSTB,
                DSTS,DSTE,SailingPT,Machine_Horse_C0_C1_VTi,idleEC,processingEC);
    }

//    public double getDSTB() {
//        return DSTB;
//    }

    public double[][] getDSTS() {
        return DSTS;
    }

    public double[][] getDSTE() {
        return DSTE;
    }

    public double[][] getMachine_Horse_C0_C1_VTi() {
        return Machine_Horse_C0_C1_VTi;
    }
    ...
    public List<WorkCenter> cloneWorkCenters() {
    ...
    public List<WorkCenter> cloneWorkCenterss(OperationOption bestOperationOption) {
        List<WorkCenter> clonedWCs = new ArrayList<>();
        for (int i = 0; i < workCenters.size(); i++) {
            if(workCenters.get(i).getHorse_C0_C1_VTi()[0]>=bestOperationOption.getUpperHorsepower())
            clonedWCs.add(workCenters.get(i));
        }
        return clonedWCs;
    }

    public List<WorkCenter> cloneWorkCenterss3(OperationOption bestOperationOption) {
        List<WorkCenter> clonedWCs = new ArrayList<>();
        for (int i = 0; i < workCenters.size(); i++) {
//            if(workCenters.get(i).getHorse_C0_C1_VTi()[0]>=bestOperationOption.getUpperHorsepower())
                clonedWCs.add(workCenters.get(i));
    ...
    public List<WorkCenter> cloneWorkCenterss33(OperationOption bestOperationOption) {
        List<WorkCenter> clonedWCs = new ArrayList<>();
        for (int i = 0; i < workCenters.size(); i++) {
            if(workCenters.get(i).getHorse_C0_C1_VTi()[0]<=
                    bestOperationOption.getUpperHorsepower()&&
                    workCenters.get(i).getHorse_C0_C1_VTi()[0]>=
                            bestOperationOption.getJob().getLowerHorsepower())
            clonedWCs.add(workCenters.get(i));
        }
        return clonedWCs;
    }

    public List<WorkCenter> cloneWorkCenterss(double upperHorsepower) {
        List<WorkCenter> clonedWCs = new ArrayList<>();
        for (int i = 0; i < workCenters.size(); i++) {
            if(workCenters.get(i).getHorse_C0_C1_VTi()[0]>=upperHorsepower)
                clonedWCs.add(workCenters.get(i));
        }
        return clonedWCs;
    }
    public List<WorkCenter> cloneWorkCenterss(Job job) {
        List<WorkCenter> clonedWCs = new ArrayList<>();
        for (int i = 0; i < workCenters.size(); i++) {
            if(workCenters.get(i).getHorse_C0_C1_VTi()[0]<=
                    job.getUpperHorsepower()&&
                    workCenters.get(i).getHorse_C0_C1_VTi()[0]>=
                            job.getLowerHorsepower())
                clonedWCs.add(workCenters.get(i));
        }
        return clonedWCs;
    }

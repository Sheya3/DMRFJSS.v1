...
import static yimei.jss.simulation.Simulation.badind;
...
//            upperbound = 5; //delete just for test

            for (int x=fp;x<upperbound;x++)
                p.evaluate(state,inds[x], pop, threadnum);
                record = badind-record;
                System.out.println(record+ "  unevaluated individuals");
                record =badind;
            }
